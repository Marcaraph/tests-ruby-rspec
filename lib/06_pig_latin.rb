    ...
    translated_words = words.map do |word|
        if word.match(/^[aeiou]/i) || (match = /^(qu|sch|[^aeiou]+qu|[^aeiou]+)(.*)/i.match(word))
            ...
            # l'expression match ? match[2] + match[1] + "ay" : word + "ay" se traduit comme suivant :
                # si le mot commence par une voyelle (/^[aeiou]/i) ou "qu" ou "sch" ou une consonne suivie par qu (comme dans square) ou plusieurs consonnes (comme dans three), alors le mot est réorganisé de manière à ce qu'il commence par une voyelle, suivi de l'ensemble des consonnes correspondant aux conditions définies, suivi de "ay"
        end
    end
